...
<div class="grid-2" style="max-width:100%;">
  <div class="card">
    <h3>Before</h3>
    <p>To promote a Card you typed <code>producer_board.py promote 32</code> in a terminal — or hand-moved it on the GitHub Projects board, which skips the spec checks and records nothing.</p>
  </div>
  <div class="card">
    <h3>After</h3>
    <p>The dashboard lists every gate that waits on you — the Card, the reason, and the exact command — with an <strong>Approve</strong> button.</p>
  </div>
</div>

<img src="./images/2026-08-28/gate-32-open-crop.png" style="max-width:96%; margin:10px auto 0; display:block; border:1px solid var(--line);" />

<p class="takeaway">The plugin still decides who may do what. <span class="accent">The dashboard only presses.</span></p>

<!--
The gates read is the plugin's own next-actions narrowed to the human's steps,
so the panel and the CLI can never disagree about whether a gate is open. The
button runs exactly the printed command through the plugin's policy checks — a
refusal (stale spec, wrong seat) comes back as the refusal text, not a
dashboard error. The command stays visible on purpose: seeing it is how a
person checks the button.
On the "hand-moved on GitHub Projects" line: the board is the source of truth,
so a manual field flip does work — the plugin follows it. But it bypasses what
promote checks (spec recorded, still current, merged) and writes no handoff
record. The button is the board-click convenience WITH the CLI's checks, which
is the whole point.
Pressed for real on 08-28 — Part 2's readiness-gate page shows the live press
and the board flipping to (Ready, dev).
Merge-type gates are deliberately behind an opt-in (AGENT_TEAMS_HUMAN_GATES=
merge); by default that button is disabled and the panel says to use the CLI —
a merge is the one destructive gate, so the safer default wins.
-->
...
<div class="grid-2" style="max-width:100%;">
  <div class="card">
    <h3>What the reviewer does</h3>
    <p>1 · Starts the app at the exact delivered commit<br/>
    2 · Walks each spec promise as a flow of <strong>2+ steps</strong><br/>
    3 · Feeds every input garbage — XSS, SQL, 3000 chars…<br/>
    4 · Reads the console <strong>after every interaction</strong></p>
  </div>
  <div class="card">
    <h3>What comes back</h3>
    <p>A <code>browser_evidence</code> block: the flows, every input case as expected-versus-actual, and the console state.<br/><br/>
    A UI change whose pass carries no such block is refused. Backend and docs cards are unaffected.</p>
  </div>
</div>
...
<p class="takeaway">Every case is written down, pass or fail — <span class="accent">expected versus actual</span>, with the console read.</p>
...
How the pass itself works, if asked: the browser worker gets the Card, the
spec, and the head SHA — never the diff. It starts the app from a detached
worktree at that exact head, walks each acceptance criterion as a named flow
of at least two steps, feeds every input field garbage, and reads the console
after each interaction, recording expected versus actual per case. The result
comes back as the browser_evidence block the verdict must carry.
-->
...
<div class="grid-2" style="max-width:100%;">
  <div class="card">
    <h3>Structure — does it fit the design?</h3>
    <p>Reads the diff against the spec's design decisions and the codebase's shape.<br/>
    <strong>Our run:</strong> search matches the spec exactly — case-insensitive substring, county never read. No findings.</p>
  </div>
  <div class="card">
    <h3>Behaviour — does it act right?</h3>
    <p>Traces correctness and edge cases on the real data, not toy examples.<br/>
    <strong>Our run:</strong> 「福壽」 finds all 50 brand records, 「中聯」 finds the one null-brand record. No defects.</p>
  </div>
  <div class="card">
    <h3>Risk — what could hurt us?</h3>
    <p>Audits the security surface, then attacks the tests themselves.<br/>
    <strong>Our run:</strong> injection-safe (all DOM writes via <code>textContent</code>) — but deleting the search wiring left 43/43 green. Finding recorded.</p>
  </div>
  <div class="card">
    <h3>Browser — does it keep the promise?</h3>
    <p>Never sees the code; drives the running app from the spec alone.<br/>
    <strong>Our run:</strong> 11 flows, 8 garbage inputs — XSS rendered as text, console clean. All pass.</p>
  </div>
</div>
