...
Part 2 is last week's live run, kept as the reference demo — none of this
week's work has run live yet.
-->
...
One thing to say out loud, because no slide says it: none of this has run live
yet. The tests show the rules are enforced. The next dataset run is what shows
they work.
-->
...
```json
"input_validation": [ { "field": "#radius", "input": "-1",
    "expected": "rejected with a range message",
    "actual":   "accepted; returned every store" } ]
```
...
The block on screen is a real defect the reviewer wrote down. Cases get recorded
