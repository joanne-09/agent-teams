...
``spec_pr_merge_mode`` and ``code_pr_merge_mode``. Old names still parse, so a
consuming repository is never broken by the rename; they are rewritten on save.
"""
...
class MergeModeRenameTests(unittest.TestCase):
    ...
    def test_legacy_names_still_parse(self):
        ...
        self.assertEqual(subject.spec_pr_merge_mode, "manual")
        self.assertEqual(subject.code_pr_merge_mode, "manual")
        self.assertEqual(subject.code_pr_merge_method, "rebase")

    def test_legacy_names_are_dropped_on_save(self):
        payload = config(spec_merge_mode="manual", merge_mode="manual").to_dict()
        self.assertNotIn("spec_merge_mode", payload)
        self.assertNotIn("merge_mode", payload)
        self.assertNotIn("merge_method", payload)
        self.assertEqual(payload["spec_pr_merge_mode"], "manual")
        self.assertEqual(payload["code_pr_merge_mode"], "manual")

    def test_new_name_wins_when_both_are_present(self):
        """A dashboard mid-migration can emit both; the current name governs."""
        subject = config(spec_merge_mode="manual", spec_pr_merge_mode="direct")
        self.assertEqual(subject.spec_pr_merge_mode, "direct")

    def test_legacy_names_are_validated_against_the_same_vocabulary(self):
        with self.assertRaisesRegex(ConfigError, "spec_pr_merge_mode"):
            config(spec_merge_mode="whenever")
    ...
    def test_renaming_alone_does_not_change_the_revision_of_equal_settings(self):
        legacy = config(spec_merge_mode="manual", merge_mode="manual")
        current = config(spec_pr_merge_mode="manual", code_pr_merge_mode="manual")
        self.assertEqual(legacy.revision, current.revision)
